...
void newArrayDescent(int *, int *, int, int, int *);
...
int main() {
    ...
    cargaVec(array1, n);
    cargaVec(array2, m);
    newArrayDescent(array1, array2, n, m, array3);
    ordenamientoSelDirecta(array3, n + m);
    printVec(array3, n + m);
}
...
void newArrayDescent(int *arr1, int *arr2, int T1, int T2, int *arr3) {
    int i, j;
    for(i = 0; i < T1; i++) {
        arr3[i] = arr1[T1 - i - 1];
    }
